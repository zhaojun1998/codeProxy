  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2">
      {images.map((image, index) => {
        ...
        if (typeof image.approxBytes === "number" && image.approxBytes > INLINE_IMAGE_MAX_BYTES) {
          return (
            <button
              key={index}
              type="button"
  ...
  return (
    <pre
      className={[
        ...
        "whitespace-pre-wrap break-words p-4 text-xs leading-relaxed font-mono dark:text-slate-200",
      ].join(" ")}
    >
      {text}
    </pre>
...
function VirtualPlainPre({ rows }: { rows: string[] }) {
  ...
  return (
    <div
      ...
      className={[
        surface({ tone: "inset", radius: "xl" }),
        "h-[min(58vh,620px)] overflow-y-auto overscroll-contain text-xs leading-relaxed font-mono [contain:layout_paint] dark:text-slate-200",
      ].join(" ")}
    >
...
function VirtualMessageList({
  ...
  useEffect(() => {
    ...
    const index = messages.findIndex(
      (message, messageIndex) => (message.messageIndex ?? messageIndex) === highlightedMessageIndex,
    );
      ...
      {open ? (
        ...
        >
          ...
          <motion.div
            ...
            aria-modal="true"
            className={[
              surface({ tone: "plain", radius: "2xl" }),
              "relative z-10 flex h-[min(82dvh,760px)] w-[min(calc(100vw-2rem),1040px)] max-w-none flex-col overflow-hidden shadow-xl",
            ].join(" ")}
            variants={{
