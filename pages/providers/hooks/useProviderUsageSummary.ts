...
import { buildCandidateUsageSourceIds, type KeyStatBucket } from "@code-proxy/domain";
import { sumStatsByCandidates } from "../providers-helpers";

type StatusBarData = import("@code-proxy/domain/usage").StatusBarData;
type StatusBlockState = import("@code-proxy/domain/usage").StatusBlockState;
type StatusBlockDetail = import("@code-proxy/domain/usage").StatusBlockDetail;
...
export function useProviderUsageSummary({
  ...
  const getOpenAIProviderStatusBar = useCallback(
    (provider: OpenAIProvider): StatusBarData =>
      buildStatusBarData(getOpenAIProviderStats(provider)),
