import type { CcSwitchImportConfigListItem } from "./ccswitchImportConfigList";

export interface CcSwitchApiKeyPermissionEntry {
...
export function ccSwitchConfigMatchesApiKeyPermissions(
  config: CcSwitchImportConfigListItem,
  entry: CcSwitchApiKeyPermissionEntry | null | undefined,
): boolean {
  if (!entry) return true;

  const entryGroups = normalizeCcSwitchPermissionList(entry["allowed-channel-groups"]).map(
